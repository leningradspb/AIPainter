...
class FullSizeWallpaperVC: UIViewController {
    ...
    private func createEasyTipView(forView: UIView, text: String, onTap: (()->Void)?) {
        ...
        textLabel.frame = CGRect(origin: CGPoint(x: 14, y: 0), size: textSize)  //CGRect(origin: .zero, size: textSize)
        ...
        let contentSize = CGSize(width: textSize.width  + closeButton.frame.width, height: textSize.height)
